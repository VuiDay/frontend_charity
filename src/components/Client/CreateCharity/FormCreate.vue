<template>
  <div class="w-[100%] flex justify-center">
    ...
    <div
      class="ml-[25px] form-create flex !px-10 !py-10 rounded-[20px] w-[60%] flex-col"
    >
      ...
      <div v-if="active === 1">
        ...
        <div class="mt-[50px]">
          ...
          <div class="w-[100%] flex justify-center mt-[20px]">
            <button class="text-white bg-[#c30047] py-3 px-5 rounded-[50px] font-semibold" @click="creaetCharity">Xác nhận thông tin</button>
          </div>
        </div>
      </div>
...
const fetchCity = async (code, name) => {
  ...
  statusCity.value = !statusCity.value;
  // consciou.value =
  //   Conscious.value.find((data) => data.code === codeConscious.value)?.name ||
  //   "";
  // arrayAddress.value = `${consciou.value}, ${city.value}, ${ward.value}`;
};
...
const saveData = (name) => {
  ...
  statusWard.value = !statusWard.value;
  // ward.value =
  //   Wards.value.find((data) => data.code === codeWards.value)?.name || "";
...
const creaetCharity = async () => {
  ...
  await store.fetchCreateCharity(formData);
  if (store.statusCrCompaign) {
  }
};
...
.form-create {
  ...
  /* box-shadow: rgba(11, 55, 231, 0.24) 0px 30px 90px; */
}
select {
  border: none;
  outline: none;
